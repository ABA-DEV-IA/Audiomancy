...
class TestBlobOperationsAsync:

    @pytest.mark.asyncio
    async def test_create_container_if_not_exists(self, mock_blob_service):
        ...
        await blob_tools.create_container_if_not_exists("test-container")
        mock_container_client.create_container.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_upload_blob_overwrite_false(self, mock_blob_service):
        mock_container_client = AsyncMock()
        mock_blob_service.get_container_client.return_value = mock_container_client
        ...
        await blob_tools.upload_blob("cache/test.json", "data", overwrite=False)
        mock_blob_client.upload_blob.assert_awaited_once_with("data", overwrite=False)

    @pytest.mark.asyncio
    async def test_download_blob_returns_none_if_not_exists(self, mock_blob_service):
        mock_container_client = AsyncMock()
        mock_blob_service.get_container_client.return_value = mock_container_client
        mock_blob_client = AsyncMock()
        mock_container_client.get_blob_client.return_value = mock_blob_client
        ...
        result = await blob_tools.download_blob("cache/test.json")
        assert result is None
        mock_blob_client.exists.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_delete_blob_calls_delete(self, mock_blob_service):
        mock_container_client = AsyncMock()
        mock_blob_service.get_container_client.return_value = mock_container_client
        mock_blob_client = AsyncMock()
        mock_container_client.get_blob_client.return_value = mock_blob_client
        ...
        await blob_tools.delete_blob("cache/test.json")
        mock_blob_client.delete_blob.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_list_blobs_returns_names(self, mock_blob_service):
        mock_container_client = AsyncMock()
        mock_blob_service.get_container_client.return_value = mock_container_client
        ...
        result = await blob_tools.list_blobs()
