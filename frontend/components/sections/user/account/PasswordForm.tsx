...
import { isPasswordValid, doPasswordsMatch } from "@/utils/user/account"
    ...
    <Card className="bg-ombre-occulte/90 border-amethyste-magique backdrop-blur-sm">
      <CardHeader>
        <CardTitle className="text-white flex items-center">
          <Lock className="h-5 w-5 mr-2 text-eclat-ether" />
          Sécurité & Mot de passe
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        <p className="text-eclat-ether text-sm">
          Laisse les champs vides si tu ne veux pas changer ton mot de passe.
        </p>

        {/* Nouveau mot de passe */}
        <div className="space-y-2">
          <label className="text-eclat-ether text-sm font-medium">Nouveau mot de passe</label>
          ...
          <div className="text-xs text-eclat-ether/80">
            {newPassword &&
              (isPasswordValid(newPassword) ? (
                ...
                <span className="text-rose-alchimique flex items-center">
        ...
        {/* Confirmation */}
        <div className="space-y-2">
          <label className="text-eclat-ether text-sm font-medium">Confirmer le nouveau mot de passe</label>
          <div className="relative">
            <Lock className="absolute left-3 top-1/2 transform -translate-y-1/2 text-amethyste-magique h-5 w-5" />
            <Input
